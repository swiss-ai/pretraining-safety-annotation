...
----------                             ---------------------------------
python -m pipeline.phase4 submit       env_command (shell preamble):
...
After all tasks complete:
  python -m pipeline.phase4 merge --run reflections
...
uv run python -m pipeline.phase4 submit --run reflections phase4.max_rows=0

# Re-run failed ranks (clears completion markers for ranks with failures)
uv run python -m pipeline.phase4 rerun --run reflections
...
uv run python -m pipeline.phase4 merge --run reflections --allow-missing
```
