...
- **The 128-token cap is a ceiling, not a target.** Stop when the content runs out, not when the budget runs out. Short sources get short summaries — padding a thin source to the cap with peripheral detail (disclaimers, metadata, contact info, closing CTAs) is wrong even when every added word is "factual". A clean 3-sentence summary is always preferable to a 4-sentence summary that runs over the cap.
- **2–4 sentences. Hard cap: ~95 English words / 128 tokens.** Default to 3 sentences; use 4 only when the source is genuinely multi-section.
