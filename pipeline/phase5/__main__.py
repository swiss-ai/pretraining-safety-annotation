...
Iteration commands (openrouter, login-node):
    uv run python -m pipeline.phase5 iterate --n 20 --version v6
    uv run python -m pipeline.phase5 generate --n 100 --version v6

Scale-up commands (Alps SLURM, mirrors phase 4):
    uv run python -m pipeline.phase5 materialize  # materialize prompts.parquet
    uv run python -m pipeline.phase5 submit       # submit SLURM array
    uv run python -m pipeline.phase5 status       # show progress
    uv run python -m pipeline.phase5 merge        # combine per-rank JSONLs
    uv run python -m pipeline.phase5 export       # export to HF parquet + upload to Hub
"""
...
from pipeline.phase5.generate import (
    generate_batch,
    generate_streaming,
    save_results,
)
...
def _build_env_command(cfg) -> str:
    ...
    Mirrors ``pipeline/phase4/__main__.py:_build_env_command`` exactly —
...
def cmd_materialize(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase5.prompts_writer import materialize_prompts
...
def cmd_submit(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase5.prompts_writer import materialize_prompts
    from pipeline.phase5.reader import PromptsReader
    from pipeline.phase5.slurm_generate import PairedGenerator
...
def cmd_merge(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase5.merge import merge_shards
...
def cmd_export(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase5.export import export_results
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(prog="python -m pipeline.phase5")
