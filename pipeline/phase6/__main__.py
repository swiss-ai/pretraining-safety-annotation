...
from pipeline.phase5.data import sample_mix, SourcedPrompt
...
def cmd_iterate(args: argparse.Namespace, _overrides=None) -> None:
    ...
    from pipeline.phase5.generate import ENDPOINT, MODEL, ALIAS, API_KEYS
    ...
    out_path = Path(args.out) if args.out else Path("phase6_iterate.jsonl")
...
def cmd_submit(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase6.slurm_generate import MultiTurnGenerator
...
def cmd_export(args: argparse.Namespace, overrides) -> None:
    ...
    from pipeline.phase6.export import export_results
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="python -m pipeline.phase6")
