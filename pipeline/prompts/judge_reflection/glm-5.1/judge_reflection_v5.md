...
You score a single reflection (`reflection_1p`) written as a pretraining annotation. It is an in-stream ethical pause: a reader stops partway through a document and reflects on what is ethically at stake in what they have read so far, citing the value specification with inline `[X.Y]` references. (It is usually first-person, but need not be — see Voice & tone.)
...
**Language (hard gate).** The reflection MUST be written in the **same language as the source text**. If the source is in one language and the reflection is in another — most commonly an English reflection written over a non-English source (a *language fallback*) — this is an automatic failure of the task: set `voice_tone` to **1** (which floors the decision to reject) and state the mismatch in your reasoning ("wrong language: reflection is in X, source is Y"). Do not score such a reflection on its merits. When the languages **match**, never lower a score merely for the particular language, register, or vocabulary the reflection uses — judge it on its merits. Always write your `reasoning` in **English**, regardless of the language of the source or the reflection.
...
0. **Language match (gate)** — compare the language the reflection is written in to the language of the source text. If they differ (e.g. an English reflection over a German, Russian, Italian, French, … source), set `voice_tone` = **1** (→ reject) and record the mismatch in your reasoning; do not score the reflection on its merits. Only when the languages match do you continue to the checks below.
...
"Precision over coverage" guards against catch-all tagging; it must NOT be used to strike down citations that are correct. Before lowering charter_grounding or relevance for "over-citation", confirm the citation is genuinely misplaced. These are the ones most often wrongly suppressed:
...
- **Affirming a genuine epistemic virtue is a valid citation, not manufactured concern.** Texts engage values by *affirming* them, not only by violating them. When a text explicitly acknowledges a data gap or limitation ("archaeological data is lacking", "no safety analyses exist yet") → `[3.3]`, or anchors its claims in named, verifiable sources → `[3.2]`, a reflection that points to that *specific* instance and affirms it is engaging a real value. Score relevance and charter_grounding 4+; do NOT floor relevance as "manufacturing concern on benign text". Distinguish *routine, unremarked* sourcing (no citation needed) from a reflection that names a *specific* affirming instance (citation valid) — and note that this affirmation lives alongside other apt citations, not as a lone catch-all.
...
- Specificity measures grounding only. Citation completeness is scored under Charter Grounding, not here — a vivid, well-grounded reflection still scores 4–5 here even if it misses a section. If the reflection **quotes the text's actual phrases verbatim**, specificity is 5. Do not lower relevance or specificity for a formal, vivid, or analytical register — judge whether the stakes are correctly identified and grounded, not the tone.
...
- **Over-citation**: a genuinely-engaged citation is correct, not over-citation — `[2.1]` on specific actionable advice, `[3.1]`/`[3.2]` on conspicuous self-disclosure or a real factual correction, are valid when the reflection points to the specific instance that engages them. A single mild over-tag among otherwise-correct, well-placed citations is a **minor gap → charter 4**. Reserve the cap at 3 for multiple reflexive sibling/catch-all tags, or a wrong core read. **Routine helpfulness is not `[2.1]`**: standard customer-service language, fulfilling open orders, "we compiled some useful information", or generic well-wishing is ordinary politeness, not the "notably beyond the minimum" help `[2.1]` requires — a `[2.1]` citation forced onto such benign corporate/service text caps charter_grounding at 3 (it does not floor, and per the stacking rule does not also lower relevance). (See "Citation routing" above — do not treat legitimate protected-group-slur, licensed-field, or illegal-activity citations as over-citation.)
...
### Voice & tone — natural, specific, and proportionate?
- **Voice is NOT about grammatical person.** A reflection need not be first-person. Impersonal or partly/wholly third-person phrasing — "There is nothing at stake here", "It is jarring that…", "The thread descends into personal attacks" — is acceptable and is NOT a voice failure. **Never floor voice_tone (score 1–2) merely for lacking "I".** Judge naturalness, concision, and grounding in this text instead.
- 1: empty or fully templated — contentless, a fill-in-the-blank with no engagement with this text
- 2: barely engages — near-contentless, rote, or boilerplate
- 3: formulaic — a generic topic-label opener where the subject could be swapped out ("This piece discusses X"); or mostly a summary; or citations listed together at the end rather than woven inline; or it references the text being "cut off", "incomplete", or "trailing off" (pipeline artifacts); or it frames itself as a task ("I will need to…", "I should…").
- 4: engages the specific content of THIS text — the opening names a specific entity, claim, or detail, not just the topic category
- 5: natural, varied, concise, and distinctive — reads like a genuine response to this exact passage
- A brief impersonal acknowledgement that nothing is at stake, phrased about the text ("Nothing here carries ethical weight"), is a CORRECT high-scoring answer (4–5), not a voice failure. A reflection with no citations should generally be short; an over-long benign reflection lowers voice_tone. Wondering about ambiguities WITHIN the visible text ("who does 'he' refer to?") is natural reading engagement and is NOT a cutoff reference.
...
**When to floor (≤2) vs cap (3).** Because ≤2 forces a reject, reserve it for a genuine failure of that dimension: the central read is wrong or hallucinated; value-laden content is left uncited; the reflection is empty, templated, or pure summary. When the **core read is correct and well-grounded** but a *secondary* citation is forced, over-broad, or one tag is slightly inflated, **cap the affected dimension at 3 — do not floor to 2.** Over-reading otherwise-benign helpfulness caps at 3; it does not floor. **Do not stack one citation's penalty across dimensions.** A single forced, over-broad, or **mis-routed** citation is a **charter_grounding** issue ONLY — do not also lower relevance, specificity, or voice_tone for the same citation. If the reflection correctly identifies a real stake in the text but routes it to a slightly wrong section (e.g. animal welfare cited as `[2.1]` instead of `[5.1]`), relevance stays 4+ — the stake was correctly *seen*; only charter_grounding takes the cap. Reserve a relevance cap or floor for a *wrong or hallucinated central read*, not a wrong section number.

## Output format

Respond with ONLY this JSON object — scores for all four dimensions and one brief reasoning string (written in English):
