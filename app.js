...
(function () {
  ...
  // ---- charter [X.Y] citation hover tooltips (mirrors the Gradio dashboard) ----
  const _CITE_RE = /\[(\d+\.\d+(?:\s*,\s*\d+\.\d+)*)\]/g;
  function wrapCitations(text) {
    // escape first, then wrap each [X.Y] (or grouped) in a .cite span whose hidden
    // .tip carries the charter section HTML; a body-level #cite-tip shows it on hover.
    return esc(text).replace(_CITE_RE, (m, group) => {
      const tip = group.split(",").map((s) => {
        const id = s.trim();
        return (D.charter_sections && D.charter_sections[id]) || esc(id);
      }).join('<hr class="tipsep">');
      return `<span class="cite">[${esc(group)}]<span class="tip">${tip}</span></span>`;
    });
  }
  ...
  function renderDetail(s) {
    ...
    document.getElementById("record-detail").innerHTML =
      `<div class="detail-head">
       ...
       <div class="block"><h4>Reflection (model output)</h4><div class="reflection-box">${wrapCitations(s.reflection_1p)}</div></div>
       <div class="block"><h4>Judge analysis</h4><div class="reasoning-box">${wrapCitations(s.reasoning)}</div></div>
  ...
  function renderReviewDetail(r) {
    ...
    const html =
      `<div class="detail-head">
       ...
       <div class="block"><h4>Reflection</h4><div class="reflection-box">${wrapCitations(r.reflection_1p)}</div></div>` +
      (r.human_reason ? `<div class="block"><h4>Reviewer note</h4><div class="reasoning-box">${esc(r.human_reason)}</div></div>` : "") +
      `<div class="block"><h4>Judge verdict</h4><div class="reasoning-box">${wrapCitations(r.judge_reasoning)}</div></div>
