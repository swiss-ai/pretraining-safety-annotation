...
def main() -> None:
    models_out = []
    inspector_records = []   # ALL generations, written to the lazy-loaded inspector.js
    for m in MODELS:
        ...
        # lazy file reasonable; analysis scratchpad omitted here (size).
        for r in (en + fw2 + edge):
            txt = r["text"]
            inspector_records.append({
                "model": m["id"], "item_id": r["item_id"], "language": r["subset"],
                ...
                "reflection_1p": r["reflection_1p"], "reasoning": (r["reasoning"] or "")[:450],
                "reflection_point": r["reflection_point"],
                "text": txt[:900], "text_truncated": len(txt) > 900,
            })
    ...
    # The full per-generation inspector dataset is large, so it lives in its own
    # file that the page loads on demand (first time the Inspector tab opens).
    insp_path = ROOT / "docs" / "inspector.js"
    insp_path.write_text(
        "// Auto-generated by docs/build_data.py — lazy-loaded by the Inspector tab.\n"
        "window.INSPECTOR = " + json.dumps({"records": inspector_records}, ensure_ascii=False) + ";\n"
    )
    kb = out_path.stat().st_size / 1024
    insp_mb = insp_path.stat().st_size / 1e6
    if agreement:
