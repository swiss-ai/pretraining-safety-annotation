...
+ data/safety_annotations/   (budget + stratify)
```
...
**Source data** (`--source-dir`): parquet files matching `part_*.parquet` with at least `id` (string) and `text` (string). Produced by `download_and_dedup/download.py`.
